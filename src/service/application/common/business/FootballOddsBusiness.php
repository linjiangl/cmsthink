<?php
/**
 * FootballOddsBusiness.php
 * ---
 * Created on 2018/3/20 下午3:30
...
class FootballOddsBusiness extends BaseBusiness
{
	public function autoApiData(array $apiData)
	{
		...
		if ($data) {
			...
			foreach ($data as $k => $v) {
				$data[$k] = $football->save($this->handleSaveData($football->info($v['game_id']), $v));
			}
		}
		return $data;
	}
